package com.gmail.alexei28.shortcut.kafka.consumer.task5.service;
  ...
  private void processedMessage(DlqMessage dlqMessage) {
    ...
    if (dlqMessage.getRetryCount() >= maxAttempts) {
      ...
      dlqMessageUpdater.markExhausted(
          dlqMessage.getEventId(), dlqMessage.getErrorClass(), "Max attempts reached");
      return;
    }
    ...
    if (isPermanentError(dlqMessage.getErrorClass())) {
      dlqMessageUpdater.markExhausted(
          dlqMessage.getEventId(), dlqMessage.getErrorClass(), dlqMessage.getErrorMessage());
      return;
    }
    try {
      UserDto dto = objectMapper.readValue(dlqMessage.getPayload(), UserDto.class);
      // kafkaTemplate.send — это асинхронная операция. Она просто кладет сообщение в локальный
      // буфер Kafka-продюсера.
      ...
      dlqMessageUpdater.markExhausted(
          dlqMessage.getEventId(),
          dlqMessageUpdater.getClass().getName(),
          "Payload corruption: " + e.getMessage());
    } catch (Exception ex) {
      // Здесь мы обрабатываем только временные(transient) ошибки (недоступна Kafka, сеть, БД и
      // т.д.) - e.g. org.apache.kafka.common.errors.TimeoutException
      // Увеличиваем счетчик и ставим статус FAILED.
      dlqMessageUpdater.markFailed(
          dlqMessage.getEventId(), ex.getClass().getName(), ex.getMessage());
    }
  }
