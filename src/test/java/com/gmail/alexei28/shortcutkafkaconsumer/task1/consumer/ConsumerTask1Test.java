...
class ConsumerTask1Test {
  ...
  */
  ...
  void setUp() throws JsonProcessingException {
    ...
    consumedTask1Content = "Task1_CONSUMED_" + consumedTask1Number;
  ...
  @Test
  @DisplayName("Should consume message exactly once (strategy at-most-once)")
  void shouldConsumeExactlyOnce() {
    // Act
    kafkaTemplate.send(topic, producedTask1Dto);

    // Assert: Ждем, когда сообщение будет обработано хотя бы один раз
    await()
        .atMost(5, SECONDS)
        .untilAsserted(
            () -> {
              // Проверяем, что вызов БЫЛ.
              // Используем times(1) здесь только если мы уверены, что Kafka не ретраит
              verify(consumerTask1, times(1)).consume(any(), any());
            });

    verifyNoMoreInteractionsAfterWait();
  }
  ...
  private void verifyNoMoreInteractionsAfterWait() {
    await()
        .atMost(3, SECONDS)
        .untilAsserted(
            () -> {
